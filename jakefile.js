...
console.log('Funtown C# SDK v' + config.version + ' (' + config.fileVersion + ')')
...
namespace('build', function () {
    ...
    task('net45', ['assemblyinfo:funtown'], function () {
        msbuild({
            file: 'Source/Funtown-Net45.sln',
            targets: ['Build']
        })
    }, { async: true })

    desc('Build .NET 4.0 binaries')
    task('net40', ['assemblyinfo:funtown'], function () {
        msbuild({
            file: 'Source/Funtown-Net40.sln',
            targets: ['Build']
        })
    }, { async: true })

    desc('Build .NET 3.5 binaries')
    task('net35', ['assemblyinfo:funtown'], function () {
        msbuild({
            file: 'Source/Funtown-Net35.sln',
            targets: ['Build']
        })
    }, { async: true })

    desc('Build WinRT(Metro) binaries')
    task('winrt', ['assemblyinfo:funtown'], function () {
        msbuild({
            file: 'Source/Funtown-WinRT.sln',
            targets: ['Build']
        })
    }, { async: true })

    desc('Build Windows Phone 7.1 binaries')
    task('wp71', ['assemblyinfo:funtown'], function () {
        msbuild({
            file: 'Source/Funtown-WP7.sln',
            targets: ['Build']
        })
    }, { async: true })

    desc('Build Silverlight 5 binaries')
    task('sl5', ['assemblyinfo:funtown'], function () {
        msbuild({
            file: 'Source/Funtown-SL5.sln',
            targets: ['Build']
        })
    }, { async: true })
...
namespace('tests', function () {
    ...
    task('net40', ['build:net40'], function () {
        xunit({
            assembly: 'Bin/Tests/Release/Funtown.Tests.dll'
        })
    }, { async: true })
...
namespace('nuget', function () {
    ...
    namespace('pack', function () {
        ...
        task('nuget', ['Dist/NuGet', 'build'], function () {
            nuget.pack({
                nuspec: 'Build/NuGet/Funtown/Funtown.nuspec',
        ...
        task('symbolsource', ['Dist/SymbolSource', 'build'], function () {
            nuget.pack({
                nuspec: 'Build/SymbolSource/Funtown/Funtown.nuspec',
                version: config.fileVersion,
    ...
    namespace('push', function () {
        ...
        task('nuget', function(apiKey) {
            nuget.push({
                apiKey: apiKey,
                package: path.join(config.rootPath, 'Dist/NuGet/Funtown.' + config.fileVersion + '.nupkg')
            })
        }, { async: true })
        ...
        task('symbolsource', function(apiKey) {
            nuget.push({
                apiKey: apiKey,
                package: path.join(config.rootPath, 'Dist/SymbolSource/Funtown.' + config.fileVersion + '.nupkg'),
...
namespace('assemblyinfo', function () {
    ...
    task('funtown', function () {
        assemblyinfo({
            file: 'Source/Funtown/Properties/AssemblyInfo.cs',
            assembly: {
                notice: function () {
                    return '// Do not modify this file manually, use jakefile instead.\r\n';
                },
                AssemblyTitle: 'Funtown',
                AssemblyDescription: 'Funtown C# SDK',
                AssemblyCompany: 'The Outercurve Foundation',
                AssemblyProduct: 'Funtown C# SDK',
