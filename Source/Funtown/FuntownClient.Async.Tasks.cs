//-----------------------------------------------------------------------
// <copyright file="FuntownClient.Async.Tasks.cs" company="The Outercurve Foundation">
...
#if ASYNC_AWAIT
, IProgress<FuntownUploadProgressChangedEventArgs> uploadProgress
#endif
)
        {
...
#if ASYNC_AWAIT
                EventHandler<FuntownUploadProgressChangedEventArgs> uploadProgressHandler = null;
                    ...
                    uploadProgressHandler = (sender, e) =>
                                            {
                                                if (e.UserState != tcs)
                                                    return;
                                                uploadProgress.Report(new FuntownUploadProgressChangedEventArgs(e.BytesReceived, e.TotalBytesToReceive, e.BytesSent, e.TotalBytesToSend, e.ProgressPercentage, userState));
                                            };
            ...
            EventHandler<FuntownApiEventArgs> handler = null;
            handler = (sender, e) =>
            {
        ...
        private void RemoveTaskAsyncHandlers(HttpMethod httpMethod, EventHandler<FuntownApiEventArgs> handler)
        {
            switch (httpMethod)
            {
                case HttpMethod.Get:
...
#endif
        /// <returns>The json result task.</returns>
        public virtual Task<object> PostTaskAsync(string path, object parameters, object userState, CancellationToken cancellationToken, IProgress<FuntownUploadProgressChangedEventArgs> uploadProgress)
        {
